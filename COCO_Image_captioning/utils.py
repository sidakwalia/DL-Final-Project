    ...
    for pos_embed_key in ("vision_pos_embed", "pos_embed", "beit3.encoder.embed_positions.A.weight"):
        if pos_embed_key in checkpoint_model:
            ...
            if orig_size != new_size:
                ...
                if torchscale_model:
                    extra_tokens = pos_embed_checkpoint[:num_extra_tokens].unsqueeze(0).to(device)
                    # only the position tokens are interpolated
                    pos_tokens = pos_embed_checkpoint[num_extra_tokens:].to(device)
                else:
                    extra_tokens = pos_embed_checkpoint[:, :num_extra_tokens].to(device)
                    # only the position tokens are interpolated
                    pos_tokens = pos_embed_checkpoint[:, num_extra_tokens:].to(device)
...
def dump_predictions(args, result, file_suffix):
    ...
    print(f"Global rank for dumping predictions: {global_rank}")
    jsons = None
    if global_rank >= 0:
    ...
    # print(f"Result file obtained is {result_file}")

    # print(f'jsons is {jsons}')
...
def coco_caption_eval(gt_dir, results_file, split):
    ...
    print(f"Annotation file is {gt_dir+'/'+annotation_file}")
    print(f"Results file is {results_file}")
    
    # Fixing annotation and result file by matching the keeps between twos
    ...
    for item in annon["annotations"]:
        if item["image_id"] in image_ids:
            final_anon["annotations"].append(item)
    
    for item in annon["images"]:
        if item["id"] in image_ids:
            final_anon["images"].append(item)
    
    with open(annotation_file, "w") as fp:
        json.dump(final_anon, fp)
